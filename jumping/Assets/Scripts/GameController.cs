...
public class GameController : MonoBehaviour {
	...
	public Vector3 spawnOutside; 				// The spawning platforms
	...
	IEnumerator SpawnWaves() { 					// IEnumeratmor - countine is getting called in the beginning of the script 
		while (true) {
			InstantiatePlat ();					// Instantiate the platform

			yield return new WaitForSeconds (spawnWait);
		}
	}

	GameObject InstantiatePlat() {				// Instantiate the platform
		Vector3 spawnOutside = new Vector3 (0.5f, 1.1f, 10.0f);	// Spawn outside = it is spawning outside the cameras view
